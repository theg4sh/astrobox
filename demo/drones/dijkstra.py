...
class Dijkstra:
    ...
    def update_units(self, func=None):
        if func is None:
            func = lambda a: True
        #units = self._unit.elerium_stocks
    ...
    def calc_weights(self, func=None):
        ...
        for f, a in enumerate(self._points):
            ...
            dump.append("%s %s"%(self._unit.id, ",".join(["%8.2f"%d if d<float("inf") else "%8s"%("") for d in self._weights[f]])))
            dump.append("")
        #print("\n".join(dump))
    ...
    def find_path(self, pt_from, pt_to, as_objects=False, info=None):
        ...
        if info:
            info = ["[{}:{}] {}->{} U:{} M:{}".format(info, self._unit.id, fi, fo, self._unit, self._unit.mothership()),]
        if fi == fo:
            if as_objects:
        ...
        if info:
            for k,t in enumerate(table):
                info.append("        {}\t{}\t{}".format(k, t, self._unit.mothership().distance_to(self._points[k])))
        path = []
        ...
        if info:
            ...
            for _,t in enumerate(path):
                info.append("        {}\t{}\t{}".format(t, self._points[t], self._unit.mothership().distance_to(self._points[t])))
            print("\n".join(info))
