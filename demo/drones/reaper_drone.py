...
class Dijkstra:
    ...
    def update_units(self, func=None):
        ...
        units = units + [m for m in self._unit.scene.motherships if not m.is_alive and m.team != self._unit.team and func(m)]
...
class ReaperStrategy(Strategy):
    ...
    def getHarvestTarget(self):
        ...
        fat_source.sort(key=lambda a: a.cargo.payload/a.scene.max_elerium + self.unit.mothership().distance_to(a)/k, reverse=True)
        if not fat_source:
            return None
